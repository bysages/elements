...
- 🖋 **纸与墨（以光为影）** — 界面是温暖的纸，内容是墨，层级由光承载。主操作默认墨色，可切换矿物颜料主题；语义色是固定的传统颜料；控件如印章般方切，器物保持圆润。这套语言桥接中式克制（留白）与西方极简，默认清晰易读，对比度与密度分层同时服务年长与年轻用户。
...
为主题赋值——主题引擎由同一个包导出，首个组件导入会把令牌层一并带入：
